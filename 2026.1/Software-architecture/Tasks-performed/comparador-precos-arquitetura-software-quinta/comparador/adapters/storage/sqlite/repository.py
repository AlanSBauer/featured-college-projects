...
class SqliteProductRepository(ProductRepository):
    ...
    def get_lowest_price_history(self, product_id: UUID) -> list[dict]:
        sql = """
        SELECT DATE(ps.fetched_at) as date, MIN(ps.price) as lowest_price
        FROM listings l
        JOIN price_snapshots ps ON ps.listing_id = l.id
        WHERE l.product_id = ?
          AND l.link_status IN ('auto', 'confirmed')
          AND ps.price IS NOT NULL
        GROUP BY DATE(ps.fetched_at)
        ORDER BY date
        """
        with self._conn() as conn:
            return [{"x": r["date"], "y": r["lowest_price"]} for r in conn.execute(sql, (str(product_id),)).fetchall()]
