...
def public_product(request: Request, product_id: str):
    ...
    lowest_price_history = repo.get_lowest_price_history(pid)
    history_series = [{"label": "Menor Preço", "data": lowest_price_history}]
    return TEMPLATES.TemplateResponse(
        request,
        "public/product.html",
        _ctx(request, product=product, listings=listings, history_json=json.dumps(history_series).replace("</", "<\\/")),
    )
